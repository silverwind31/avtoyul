...
/**
 * This is the model class for table "report_ruxsatnoma".
 *
 * @property int $id
 ...
 */
class ReportRuxsatnoma extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    public static function getCount($user_id = NULL, $month_id = NULL){
        ...
        if(!empty($user_id)){
            ...
            $reports = ArrayHelper::map(Report::find()->where(['document_type'=>5])->andWhere(['user_id'=>$user_id])->andWhere(['status'=>40])->all(),'id','id');

        }

        if(!empty($month_id)){

            $reports = ArrayHelper::map(Report::find()->where(['document_type'=>5])->andWhere(['month'=>$month_id])->andWhere(['status'=>40])->all(),'id','id');

        }

        if(!empty($user_id) && !empty($month_id)){

            $reports = ArrayHelper::map(Report::find()->where(['document_type'=>5])->andWhere(['user_id'=>$user_id])->andWhere(['month'=>$month_id])->andWhere(['status'=>40])->all(),'id','id');

        }

        $count = ReportRuxsatnoma::find()->where(['report_id' => $reports])->count();
