...
/**
 * This is the model class for table "report_uyjoy_ruxsatnoma".
 *
 * @property int $id
 ...
 */
class ReportUyjoyRuxsatnoma extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return 'report_uyjoy_ruxsatnoma';
    }
    ...
    public static function getCount($user_id = NULL, $month_id = NULL){
        ...
        if(!empty($user_id)){
            ...
            $reports = ArrayHelper::map(Report::find()->where(['document_type'=>6])->andWhere(['user_id'=>$user_id])->andWhere(['status'=>40])->all(),'id','id');

        }

        if(!empty($month_id)){

            $reports = ArrayHelper::map(Report::find()->where(['document_type'=>6])->andWhere(['month'=>$month_id])->andWhere(['status'=>40])->all(),'id','id');

        }

        if(!empty($user_id) && !empty($month_id)){

            $reports = ArrayHelper::map(Report::find()->where(['document_type'=>6])->andWhere(['user_id'=>$user_id])->andWhere(['status'=>40])->andWhere(['month'=>$month_id])->all(),'id','id');

        }

        $count = ReportUyjoyRuxsatnoma::find()->where(['report_id' => $reports])->count();
