...
$this->title = Yii::t('main', 'Update Contact Subject');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Contact Subjects'), 'url' => ['index']];
...
{
    $lang_model = \common\models\ContactSubjectLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ContactSubjectLang();
        $action = "/contact-subject-lang/create";

    } else {

        $lang_model = \common\models\ContactSubjectLang::findOne($lang_model->id);
        $action = "/contact-subject-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/contact-subject-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
