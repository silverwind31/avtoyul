...
{
    $lang_model = \common\models\ProductCategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ProductCategoryLang();
        $action = "/product-category-lang/create";

    } else {

        $lang_model = \common\models\ProductCategoryLang::findOne($lang_model->id);
        $action = "/product-category-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/product-category-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
