...
$this->title = Yii::t('main', 'Update Product');
...
{
    $lang_model = \common\models\ProductLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ProductLang();
        $action = "/product-lang/create";

    } else {

        $lang_model = \common\models\ProductLang::findOne($lang_model->id);
        $action = "/product-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
    	'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/product-lang/_form', [
        	'id' => $language->id,
        	'model' => $lang_model,
        	'action' => $action,
        ]),
