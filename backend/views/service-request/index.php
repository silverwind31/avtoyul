...
$this->title = Yii::t('main', 'Service Requests');
...
<!--                        --><?//= Html::a( Yii::t('main','Create Service Request'), ['create'], ['class' => 'btn btn-success']) ?>
...
<?= GridView::widget([
                            ...
                            'columns' => [
                                ...
                                [
                                    ...
                                    'contentOptions' => ['class' => 'v-align-middle'],
                                    'filter' => Html::activeDropDownList($searchModel, 'service_id', Arrayhelper::map(Services::find()->orderBy('id')->asArray()->all(), 'id', 'title' ), ['class'=>'full-width', 'data-init-plugin' => 'select2','prompt' => Yii::t('main', 'Select Category')]),
                                    'content' => function($model)
                                    {
                                        if( isset( $model->service->title ) )
                                            return StaticFunctions::getPartOfText( $model->service->title, 50 );
                                    }
                                ],
