...
$this->title = Yii::t('main', 'Update Book');
...
{
    $lang_model = \common\models\BooksLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\BooksLang();
        $action = "/books-lang/create";

    } else {

        $lang_model = \common\models\BooksLang::findOne($lang_model->id);
        $action = "/books-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/books-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
