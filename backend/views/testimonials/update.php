...
$this->title = Yii::t('main', 'Update Testimonial');
...
{
    $lang_model = \common\models\TestimonialsLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\TestimonialsLang();
        $action = "/testimonials-lang/create";

    } else {

        $lang_model = \common\models\TestimonialsLang::findOne($lang_model->id);
        $action = "/testimonials-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/testimonials-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
