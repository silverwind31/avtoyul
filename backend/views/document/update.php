...
$this->title = Yii::t('main', 'Update Document');
...
{
    $lang_model = \common\models\DocumentLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\DocumentLang();
        $action = "/document-lang/create";

    } else {

        $lang_model = \common\models\DocumentLang::findOne($lang_model->id);
        $action = "/document-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/document-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
