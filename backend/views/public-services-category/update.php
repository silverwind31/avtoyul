...
$this->title = Yii::t('main', 'Update Public Services Category');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Public Services Categories'), 'url' => ['index']];
...
{
    $lang_model = \common\models\PublicServicesCategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\PublicServicesCategoryLang();
        $action = "/public-services-category-lang/create";

    } else {

        $lang_model = \common\models\PublicServicesCategoryLang::findOne($lang_model->id);
        $action = "/public-services-category-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/public-services-category-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
