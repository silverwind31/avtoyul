...
$this->title = Yii::t('main', 'Update Partner');
...
{
    $lang_model = \common\models\PartnerLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\PartnerLang();
        $action = "/partner-lang/create";

    } else {

        $lang_model = \common\models\PartnerLang::findOne($lang_model->id);
        $action = "/partner-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/partner-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
