...
$this->title = Yii::t('main', 'Update Region');
...
{
    $lang_model = \common\models\RegionLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\RegionLang();
        $action = "/region-lang/create";

    } else {

        $lang_model = \common\models\RegionLang::findOne($lang_model->id);
        $action = "/region-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/region-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
