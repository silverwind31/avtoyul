...
$this->title = Yii::t('main', 'Update Banner');
...
{
    $lang_model = \common\models\BannerLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\BannerLang();
        $action = "/banner-lang/create";

    } else {

        $lang_model = \common\models\BannerLang::findOne($lang_model->id);
        $action = "/banner-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/banner-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
