...
$this->title = Yii::t('main', 'Update Report Offense');
...
{
    $lang_model = \common\models\Report OffensesLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\Report OffensesLang();
        $action = "/services-lang/create";

    } else {

        $lang_model = \common\models\Report OffensesLang::findOne($lang_model->id);
        $action = "/Report Offenses-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/Report Offenses-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
