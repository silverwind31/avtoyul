...
$this->title = Yii::t('main', 'Update Menu');
...
{
    $lang_model = \common\models\MenuLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\MenuLang();
        $action = "/menu-lang/create";

    } else {

        $lang_model = \common\models\MenuLang::findOne($lang_model->id);
        $action = "/menu-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/menu-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
