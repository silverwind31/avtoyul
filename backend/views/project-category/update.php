...
$this->title = Yii::t('main', 'Update Project Category');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Project Categories'), 'url' => ['index']];
...
{
    $lang_model = \common\models\ProjectCategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ProjectCategoryLang();
        $action = "/project-category-lang/create";

    } else {

        $lang_model = \common\models\ProjectCategoryLang::findOne($lang_model->id);
        $action = "/project-category-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/project-category-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
