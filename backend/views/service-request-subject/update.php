...
$this->title = Yii::t('main', 'Update Subject');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Request Subjects'), 'url' => ['index']];
...
{
    $lang_model = \common\models\ServiceRequestSubjectLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ServiceRequestSubjectLang();
        $action = "/service-request-subject-lang/create";

    } else {

        $lang_model = \common\models\ServiceRequestSubjectLang::findOne($lang_model->id);
        $action = "/service-request-subject-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/service-request-subject-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
