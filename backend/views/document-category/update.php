...
$this->title = Yii::t('main', 'Update Document Category');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Document Categories'), 'url' => ['index']];
...
{
    $lang_model = \common\models\DocumentCategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\DocumentCategoryLang();
        $action = "/document-category-lang/create";

    } else {

        $lang_model = \common\models\DocumentCategoryLang::findOne($lang_model->id);
        $action = "/document-category-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/document-category-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
