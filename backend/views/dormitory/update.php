...
$this->title = Yii::t('main', 'Update Dormitory');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Dormitories'), 'url' => ['index']];
...
{
    $lang_model = \common\models\DormitoriesLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\DormitoriesLang();
        $action = "/services-lang/create";

    } else {

        $lang_model = \common\models\DormitoriesLang::findOne($lang_model->id);
        $action = "/Dormitories-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/Dormitories-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
