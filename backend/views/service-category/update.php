...
$this->title = Yii::t('main', 'Update Service Category');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Service Categories'), 'url' => ['index']];
...
{
    $lang_model = \common\models\ServiceCategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ServiceCategoryLang();
        $action = "/service-category-lang/create";

    } else {

        $lang_model = \common\models\ServiceCategoryLang::findOne($lang_model->id);
        $action = "/service-category-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/service-category-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
