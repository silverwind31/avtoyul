...
$this->title = Yii::t('main', 'Update Project');
...
{
    $lang_model = \common\models\ProjectLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ProjectLang();
        $action = "/project-lang/create";

    } else {

        $lang_model = \common\models\ProjectLang::findOne($lang_model->id);
        $action = "/project-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/project-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
