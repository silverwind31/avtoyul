...
$this->title = Yii::t('main', 'Update News');
...
{
    $lang_model = \common\models\NewsLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\NewsLang();
        $action = "/news-lang/create";

    } else {

        $lang_model = \common\models\NewsLang::findOne($lang_model->id);
        $action = "/news-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
    	'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/news-lang/_form', [
        	'id' => $language->id,
        	'model' => $lang_model,
        	'action' => $action,
        ]),
