...
{
    $lang_model = \common\models\CompanyLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\CompanyLang();
        $action = "/company-lang/create";

    } else {

        $lang_model = \common\models\CompanyLang::findOne($lang_model->id);
        $action = "/company-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/company-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
