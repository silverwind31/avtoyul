...
$this->title = Yii::t('main', 'Update Service');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Services'), 'url' => ['index']];
...
{
    $lang_model = \common\models\ServicesLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\ServicesLang();
        $action = "/services-lang/create";

    } else {

        $lang_model = \common\models\ServicesLang::findOne($lang_model->id);
        $action = "/services-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/services-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
