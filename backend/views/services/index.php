        ...
        <div class="panel-body no-padding">
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body page-header-block">
                       ...
                       <?= Html::a( Yii::t('main', 'Create Service'), ['create'], ['class' => 'btn btn-success']) ?>
                        ...
                        <?= GridView::widget([
                            ...
                            'columns' => [
                                ...
                                [
                                    ...
                                    'contentOptions' => ['class' => 'v-align-middle'],
                                    'content' => function($model){
                                        return htmlentities(StaticFunctions::getPartOfText($model->title, 100));
                                    }
                                ],
                                [
                                    'attribute' => 'category',
                                    'contentOptions' => ['class' => 'v-align-middle'],
                                    'filter' => Html::activeDropDownList($searchModel, 'category', Arrayhelper::map(\common\models\ServiceCategory::find()->orderBy('id')->asArray()->all(), 'id', 'name'),['class'=>'full-width', 'data-init-plugin' => 'select2','prompt' => Yii::t('main', 'Select Category')]),
                                    'value' => function($data){

                                        return $data->serviceCategory->name;
                                ...
                                [
                                    ...
                                    'content' => function($model)
                                    {
                                        if($model->image && file_exists(Yii::getAlias('@frontend') . '/web' . Yii::$app->params['uploads_url'] . Yii::$app->controller->id . '/' . $model->id . '/s_' . $model->image ))
                                        {
                                            ...
                                            return '<img class="img-responsive postImage" data-title="' . htmlentities(StaticFunctions::getPartOfText($model->title, 100)) . '" data-img="' . Yii::$app->params['frontend'] . Yii::$app->params['uploads_url'] . Yii::$app->controller->id . '/' . $model->id . '/l_' . $model->image . '" src="' . Yii::$app->params['frontend'] . Yii::$app->params['uploads_url'] . Yii::$app->controller->id . '/' . $model->id . '/s_' . $model->image . '"/>';
