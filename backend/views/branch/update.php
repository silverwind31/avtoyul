...
$this->title = Yii::t('main', 'Update Branch');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Branches'), 'url' => ['index']];
...
{
    $lang_model = \common\models\BranchLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\BranchLang();
        $action = "/branch-lang/create";

    } else {

        $lang_model = \common\models\BranchLang::findOne($lang_model->id);
        $action = "/branch-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/branch-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
