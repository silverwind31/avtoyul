...
{
    $lang_model = NewsCategoryLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new NewsCategoryLang();
        $action = "/news-category-lang/create";

    } else {

        $lang_model = NewsCategoryLang::findOne($lang_model->id);
        $action = "/news-category-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/news-category-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
