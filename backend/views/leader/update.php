...
$this->title = Yii::t('main', 'Update Leader');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'Leaders'), 'url' => ['index']];
...
{
    $lang_model = \common\models\LeaderLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\LeaderLang();
        $action = "/leader-lang/create";

    } else {

        $lang_model = \common\models\LeaderLang::findOne($lang_model->id);
        $action = "/leader-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/leader-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
