...
$this->title = Yii::t('main', 'Update District');
$this->params['breadcrumbs'][] = ['label' => Yii::t('main', 'District'), 'url' => ['index']];
...
{
    $lang_model = \common\models\DistrictLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\DistrictLang();
        $action = "/district-lang/create";

    } else {

        $lang_model = \common\models\DistrictLang::findOne($lang_model->id);
        $action = "/district-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/district-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
