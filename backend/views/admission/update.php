...
$this->title = Yii::t('main', 'Update Admission');
...
{
    $lang_model = AdmissionLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new AdmissionLang();
        $action = "/admission-lang/create";

    } else {

        $lang_model = \common\models\AdmissionLang::findOne($lang_model->id);
        $action = "/admission-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/admission-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
