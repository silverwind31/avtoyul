...
class PublicServicesController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if($model->save())
            {
                ...
                if($image)
                {
                    $model->main_image = StaticFunctions::saveImage($image, $model->id, 'publicservices');
                }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            {
                $model->main_image = StaticFunctions::saveImage($image, $model->id, 'publicservices');
                StaticFunctions::deleteImage($image_old, $model->id, 'publicservices');
    ...
    protected function saveFiles($files, $section, $parent)
    {
        foreach($files as $file)
        {
            ...
            $dir = Yii::getAlias('@frontend') . '/web' . Yii::$app->params['uploads_url'] . 'publicservices/' . $parent . '/';
